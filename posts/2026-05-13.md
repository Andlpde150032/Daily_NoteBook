...
**Mục tiêu (Target):** Windows Server 2022 (IP: `192.168.174.129`) - Domain: `AnDLP.local`
...
- **Network Setup:** Mạng ảo Host-only (Subnet `192.168.174.0/24`) để đảm bảo an toàn, cách ly hoàn toàn với mạng thật.

---

## 🛠️ Cấu hình Lab & Tiền đề khai thác (Prerequisites)

Để kịch bản tấn công này có thể diễn ra, máy chủ mục tiêu đã được cấu hình và thiết lập một số "điểm mù" bảo mật có chủ đích nhằm mô phỏng lại những sai lầm thực tế của quản trị viên:

* **Setup Active Directory:** Windows Server 2022 đã được cài đặt role **Active Directory Domain Services (AD DS)** và promote thành Domain Controller cho domain `AnDLP.local`.
* **Tài khoản cấp thấp (Initial Foothold):** Đã tạo sẵn một tài khoản nhân viên thông thường là `nhanvienIT1` (thuộc nhóm Domain Users) để mô phỏng trường hợp một nhân viên bị lộ mật khẩu hoặc bị tấn công Phishing.
* **Mồi nhử (The Bait):** Cố ý tạo một kịch bản tự động `password.bat` chứa mật khẩu cấp cao dưới dạng plaintext và đặt vào thư mục `C:\Windows\SYSVOL\sysvol\AnDLP.local\scripts` (thư mục mà mọi nhân viên đều có quyền đọc).
* **Dịch vụ từ xa:** Cổng 5985 (WinRM) được cấu hình cho phép kết nối từ xa.
...
Tiến hành lục lọi trong `SYSVOL/AnDLP.local/scripts` và phát hiện một file kịch bản tự động `password.bat`.
